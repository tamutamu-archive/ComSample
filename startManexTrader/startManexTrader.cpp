// startManexTrader.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_ABOUT:
			...
			break;
		case BUTTON2:
			// MessageBox(hWnd, L"test", L"確認", MB_OK);
			NewIE();
			break;
		case IDM_EXIT:
			DestroyWindow(hWnd);
...
void NewIE()
{
	...
	for (long i = 0;i < allFrames->length;i++)
	{
		...
		for (long i = 0;i < nFrames->length;i++)
		{
			...
			if (_tcscmp((LPCTSTR)href, _T("ネットストック・ハイスピード")) != 0)
				continue;
